...
/** Pipeline management page — the phase catalog (kanban columns / dossier stages). */
export default function PipelineManagementView({ setMobileMenuOpen }: { setMobileMenuOpen?: (o: boolean) => void }) {
  return (
    <div className="flex h-full flex-1 flex-col overflow-hidden bg-[#F5F5F7]">
      <header className="border-b border-slate-100 bg-white/80 px-4 py-4 backdrop-blur lg:px-6">
        ...
        <div>
          <h1 className="font-serif-mct text-base sm:text-xl font-bold text-[#332151]">Gestion du pipeline</h1>
          <p className="text-xs text-[#5A5A7A]">Configurer les phases d&apos;onboarding — colonnes du kanban et étapes des dossiers.</p>
        </div>
      </header>
