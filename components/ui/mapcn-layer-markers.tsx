...
type SelectedPoint = MarkerPointProperties & {
  coordinates: [number, number];
};
...
function generateRandomPoints(count: number) {
  ...
  const categories = ["Restaurant", "Cafe", "Bar", "Shop"];
  const features = [];
  for (let i = 0; i < count; i++) {
    const lng = center.lng + (seededRandom(i + 1) - 0.5) * 0.15;
    const lat = center.lat + (seededRandom(i + 401) - 0.5) * 0.1;
    features.push({
      type: "Feature" as const,
      properties: { id: i, name: i === 0 ? "Location 1" : "Location " + (i + 1), category: categories[Math.floor(seededRandom(i + 801) * 4)] },
...
function LayerMarkers() {
  ...
  useEffect(() => {
    ...
    map.addLayer({ id: layerId, type: "circle", source: sourceId, paint: { "circle-radius": 6, "circle-color": "#3b82f6", "circle-stroke-width": 2, "circle-stroke-color": "#ffffff" } });
    const handleClick = (e: MapLibreGL.MapMouseEvent & { features?: MapLibreGL.MapGeoJSONFeature[] }) => {
      if (!e.features?.length) return;
      const feature = e.features[0];
      const coords = (feature.geometry as GeoJSON.Point).coordinates as [number, number];
      setSelectedPoint({ id: feature.properties?.id, name: feature.properties?.name, category: feature.properties?.category, coordinates: coords });
    };
      ...
      {selectedPoint && (
        <MapPopup longitude={selectedPoint.coordinates[0]} latitude={selectedPoint.coordinates[1]} onClose={() => setSelectedPoint(null)} closeOnClick={false} focusAfterOpen={false} offset={10} closeButton>
          <div className="min-w-24">
            ...
            <p className="text-muted-foreground text-sm">{selectedPoint.category}</p>
          </div>
        </MapPopup>
      )}
    </>
