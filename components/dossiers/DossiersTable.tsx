  ...
  return (
    ...
    >
      ...
      <div className="p-6 border-b border-slate-100 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h4 className="text-base font-extrabold font-serif-mct text-[#2D2A56]">{totalItems} dossiers</h4>
      ...
      <Table className="min-w-[640px]">
        ...
        <TableBody>
          {displayedDossiers.map((item) => (
            <TableRow key={item.id} className="group cursor-pointer" onClick={() => onOpenDossier?.(item.id)}>
              {/* 1. Dossier identity */}
              <TableCell className="px-6">
                <h5 className="font-extrabold text-[#2D2A56] text-sm group-hover:text-[#EA5B2D] transition-colors leading-tight mb-1.5">
                  {item.centre}
                </h5>
                <div className="flex flex-wrap items-center gap-2 text-[10px] text-[#5A5A7A] font-semibold">
                  <span className="font-mono text-[#2D2A56]">
                    {item.code ?? item.id}
                  </span>
              ...
              <TableCell className="px-5">
                <span className="text-xs text-[#2D2A56] font-bold flex items-center gap-1.5">
              ...
              <TableCell className="px-6 text-right">
                <button
                  ...
                  className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-[#EA5B2D]/10 hover:text-[#EA5B2D] group-hover:text-[#EA5B2D]"
