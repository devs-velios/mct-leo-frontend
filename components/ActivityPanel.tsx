...
export default function ActivityPanel({ activities }: ActivityPanelProps) {
  ...
  const getActivityIconBadge = (type: string) => {
    switch (type) {
      ...
      case "document":
        return (
          <span className="p-2 xl:p-2.5 rounded-xl bg-[#332151]/5 text-[#332151] inline-flex items-center justify-center shrink-0 shadow-sm border border-[#332151]/5">
      ...
      case "reminder":
        return (
          <span className="p-2 xl:p-2.5 rounded-xl bg-orange-50 text-[#E34F2D] inline-flex items-center justify-center shrink-0 shadow-sm border border-orange-100/50">
      ...
      case "rag":
        return (
          <span className="p-2 xl:p-2.5 rounded-xl bg-[#E34F2D]/10 text-[#E34F2D] inline-flex items-center justify-center shrink-0 shadow-sm border border-[#E34F2D]/10">
            <Cpu className="h-3.5 xl:h-4 w-3.5 xl:w-4" />
          </span>
        );
      default:
    ...
    <div className="p-4 sm:p-6 rounded-3xl glow-effect bg-white shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col h-full w-full min-w-0 overflow-hidden">
      <div className="flex items-center justify-between mb-4 pb-3 shrink-0">
        <div>
          ...
          <h4 className="text-base font-bold font-serif-mct text-[#332151]">Activité temps réel</h4>
      ...
      <div className="flex-1 overflow-y-auto pr-2 sm:pr-3.5 custom-scrollbar min-h-0 space-y-3.5 xl:space-y-4">
        {activities.map((act) => (
          <div key={act.id} className="group flex items-start gap-3 xl:gap-4 p-3 xl:p-3.5 rounded-2xl bg-slate-50/70 hover:bg-slate-100/90 transition-all duration-200 shadow-sm border border-slate-100 hover:border-[#E34F2D]/20">
            {getActivityIconBadge(act.type)}
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[11px] xl:text-xs font-extrabold text-[#332151] truncate uppercase tracking-wider">
                  {act.centre}
                </span>
