...
export default function Sidebar({ activeTab, setActiveTab, userEmail, handleLogout }: SidebarProps) {
  ...
  return (
    <aside
      className={`hidden lg:flex flex-col bg-[#332151] text-[#F5F0E8] relative z-20 shrink-0 select-none transition-[width] duration-200 ease-in-out ${
    ...
    >
      ...
      <button
        ...
        className="absolute top-7 -right-3 z-30 flex h-6 w-6 items-center justify-center rounded-full bg-white text-[#332151] shadow-md ring-1 ring-black/5 hover:text-[#E34F2D] transition-colors cursor-pointer"
      >
      ...
      >
        ...
        {!collapsed && (
          <div className="truncate">
            ...
            <p className="text-[9px] xl:text-[10px] text-[#E34F2D] uppercase tracking-widest font-bold">
              AI Onboarding
            </p>
          </div>
        )}
            ...
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-white/5 border-l-4 border-[#E34F2D] z-0"
                  transition={{ type: "spring", stiffness: 380, damping: 30 }}
                />
              )}
              <div className={`flex items-center relative z-10 ${collapsed ? "" : "gap-3"}`}>
                <Icon className={`h-4.5 w-4.5 shrink-0 ${isActive ? "text-[#E34F2D]" : "text-[#F5F0E8]/50 group-hover:text-[#F5F0E8]"}`} />
        ...
        >
          <div className={`flex items-center truncate ${collapsed ? "" : "gap-2 xl:gap-2.5"}`}>
            <div className="relative shrink-0">
              <div className="h-8 w-8 xl:h-8.5 xl:w-8.5 rounded-full bg-[#E34F2D]/20 flex items-center justify-center font-bold text-xs text-[#E34F2D]">
                OP
              </div>
